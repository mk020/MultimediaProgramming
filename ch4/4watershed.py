...
ret, bin_img = cv.threshold(gray, 0, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU) # 이진화
#cv.imshow("binarized image", bin_img)

# white(object) noise removal
kernel = cv.getStructuringElement(cv.MORPH_RECT, (3, 3)) # 3x3
b_opening = cv.dilate(cv.erode(bin_img,kernel,iterations=2),kernel,iterations=2)	# 열기(침식2번->팽창2번)
