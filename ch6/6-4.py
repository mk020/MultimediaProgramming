...
class TrafficWeak(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('물체 인식')
        self.setGeometry(200,200,700,200)
       
        signButton=QPushButton('물체 등록',self)
        roadButton=QPushButton('물체 영상 불러옴',self)
        ...
        self.signFiles=[['bujeok.jpg','부적'],['lucky.jpg','네잎클로버'],['yellowcard.jpg','문화카드']]	# 카드 모델 영상
        self.signImgs=[]				# 표지판 모델 영상 저장
        
    def signFunction(self):
        self.label.clear()
        self.label.setText('카드를 등록합니다.')
    ...
    def roadFunction(self):
        if self.signImgs==[]: 
            self.label.setText('먼저 카드를 등록하세요.')
        else:
    ...
    def recognitionFunction(self):
        if self.roadImg is None: 
            self.label.setText('먼저 카드들 영상을 입력하세요.')
        else:
            sift=cv.SIFT_create()
            ...
            if len(GM[best])<4:	# 최선의 번호판이 매칭 쌍 4개 미만이면 실패
                ...
                box1=np.float32([[0,0],[0,h1-1],[w1-1,h1-1],[w1-1,0]]).reshape(4,1,2)  # 사각형
                box2=cv.perspectiveTransform(box1,H)  # H에 의한 임의의 사각형
