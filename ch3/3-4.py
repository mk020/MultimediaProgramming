...
b_closing=cv.erode(cv.dilate(b,se,iterations=1),se,iterations=1)	# 닫기 : 팽창 -> 침식 (검은색 잡음 없앰)
#plt.imshow(b_closing,cmap='gray'), plt.xticks([]), plt.yticks([])
#plt.show()

b_opening=cv.dilate(cv.erode(b,se,iterations=1),se,iterations=1)    # 열기 : 침식 -> 팽창 (흰색 잡음 없앰)
#plt.imshow(b_closing,cmap='gray'), plt.xticks([]), plt.yticks([])
#plt.show()
