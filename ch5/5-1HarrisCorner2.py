...
median = cv.medianBlur(img, 3)  # 스무딩 - 노이즈 제거
gray = cv.cvtColor(median, cv.COLOR_BGR2GRAY)

blockSize = 4  # 이웃 화소 범위 = 패치 사이즈
...
# thresh => 현재 trackbar의 위치, 30 => 최대값,
# 값이 변경될 때마다(trackbar 움직일 때마다) 이벤트 발생 -> onCornerHarris 함수 자동으로 호출, 실행
